...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:beverage) {described_class.new("beverage", 16)}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(beverage.type).to eq('beverage')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:drink) {described_class.new("Voka", 16)}
      # remove the 'x' before running this test
      it 'is full' do
        expect(drink.full?).to eq true
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:strongbow) {described_class.new("strongbow", 13)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(strongbow.full?).to be false
      end
    end
  end
end
